...
// O envio do email é feito pelo browser via Web3Forms (client-side).
// Este endpoint serve apenas para guardar o registo no painel admin.
app.post('/api/contact', (req, res) => {
  const { name, email, phone, subject, message, emailStatus, emailError } = req.body;
  if (!name || !email || !message) {
    return res.status(400).json({ error: 'Nome, email e mensagem são obrigatórios.' });
  }
  const entry = { name, email, phone: phone || '', subject, message, status: emailStatus || 'sent' };
